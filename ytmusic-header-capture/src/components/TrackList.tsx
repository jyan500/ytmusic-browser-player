...
export const TrackList = ({ data }: Props) => {
    ...
    return (
        <ul className="flex flex-col bg-dark-secondary text-white">
            {data?.map((track: Track) => (
                ...
                }} tabIndex={0} key={track.videoId} className={`group flex flex-row justify-between items-center p-2 ${currentTrack?.videoId === track.videoId ? "bg-orange-secondary" : ""}`}>
                    <div className = "flex flex-row gap-x-2">
